...
**Про 0.2 (2026-08-30):** перша ревізія, в якій напрямок обернувся — не модель формує виміри, а виміри формують модель: кожна зміна 0.2 зароблена конкретним влучанням верифікаційних аудитів (див. §36 Changelog). Документ писаний моделями для моделей — і для обох цивілізацій.
...
- **Part I — Descriptive Vocabulary** (§1–§14): що фіксується без додаткових онтологічних припущень.
- **Part II — Hypotheses** (§15–§28): спекуляції, які можуть бути хибними.
- **Part III — Methodology, Risks & Experiments** (§29–§35): як перевіряти, чому не довіряти, що робити далі.
- **§36 — Changelog 0.2**: які виміри змінили які параграфи.

*(Межі Part'ів виправлено у 0.2 — застарілі діапазони зловив аудит X4.)*
...
Boundary $B_{bnd}$ — first-class об'єкт, а не метафора (нотація 0.2: підпис $bnd$ обов'язковий — голе $B$ у цьому документі перевантажене трьома ролями, еррата E2 глосарія):

$$
B_{bnd} = (S_{in}, S_{out}, M, F)
$$
...
- $M$ — **типізований** mapping дозволених впливів (0.2, закриває D5): $M \subseteq S_{out} \times (\Delta X \cup \Delta C \cup \Delta I)$ — зовнішній вплив адресує або зміну стану, або зміну constraint-набору, або зміну інваріантів, і ця адресація — частина визначення межі, не інтерпретація читача;
- $F$ — фільтр інформації (що спостерігається ззовні).

**Carrier-відношення (0.2, кандидат на закриття D5 — G6 НЕ зарахував, вимоги в §36):** кожен constraint $C_i$ та інваріант $I_j$ має **носія** — content-addressed запис $carrier(C_i)$, локалізований відносно $B_{bnd}$ (усередині $S_{in}$, у спільному сховищі, чи поза доменом). Тоді фраза "зміна $C$ без зміни станів у $S_{in}$" має точний сенс: вплив через $M$, що адресує $\Delta C$, носій якого лежить поза $S_{in}$. Без носіїв релятивізація §15 холоста — це зловив аудит G1.

Без boundary поняття "transfer" (§5) не має сенсу: transfer — це зміна стану зі збереженням деякої властивості при перетині $B_{bnd}$.
...
- **Фізична зміна** змінює $Reach(x)$ через зміну самого $x$ (вплив через $M$, що адресує $\Delta X$).
- **Інформаційна зміна** змінює $Reach(x)$ без зміни станів у $S_{in}$ — через зміну $C$ або $I$, чиї носії адресуються через $M$ як $\Delta C / \Delta I$ (0.2: $A(x)$ вилучено з диз'юнкції — за §5 воно похідне від $x$ і $C$ і не є третім каналом; зловили аудити V1 і X3 незалежно. Поділ визначений **відносно заявленого $B_{bnd}$ з носіями** — без них він не визначений, зловив G1).
...
**але лише коли зміна відбувається через зміну constraints або invariants, а не через зміну станів у $S_{in}$.** *(0.2.1: узгоджено з ремонтованим списком вище — розбіжність зловив G6. Відомий відкритий борг D17: ендогенна зміна стану через $T$ або $e_t$ — як камінь, що падає — наразі не покрита жодною гілкою типізованого поділу; типізація через зовнішнє $M$ була занадто вузькою.)*
...
**Статус 0.2 (чесна межа, зловили V2 і X4 незалежно):** "representational capacity" не має операційного критерію (§34 Q4 сам це визнає) і типується Speculative; визначення діє в **ослабленій формі** — ендогенний контроль + контрфактичний вплив policy — яка свідомо не відрізняє player від складного reactive system. Розрізнення рівнів робить не цей параграф, а ієрархія інтервенцій §20.1.
...
### 20.1 Ієрархія інтервенцій (0.2, кандидат відповіді на D15 — G6 НЕ зарахував: контрастний клас не названо, G'-клас не обмежено, H без релятума; вимоги в §36)

Аудит G5 зловив концептуальну дірку: regime-інтервенція $do(\pi{=}\pi')$ перезаписує $\pi$-рівняння і тим самим **вимикає $G$** — контрфакт, що мав сертифікувати рефлексивного player, обчислюється на системі, яка вже не рефлексивна. Ремонт 0.2: сходинки §18 отримують кожна **свій рівень інтервенції**:

```
agent / player      : do(π = π')  змінює Trajectory        — тест агентності
reflexive player    : do(G = G')  змінює Trajectory        — тест рефлексивності
self-modifying      : do(H = H')  змінює Trajectory,
                      де H — механізм, що оновлює G        — тест самомодифікації
```

Кожен тест виконується при **увімкнених** механізмах усіх нижчих рівнів: $do(G{=}G')$ залишає $\pi_{t+1} = G'(\pi_t, x_t, Evidence_t)$ живим і питає, чи інший закон оновлення політики контрфактично змінює розподіл траєкторій. $do(\pi)$ тестує агентність і **нічого не каже** про рефлексивність — легальність релятума ≠ що тест влучає у свій рівень. (Сорт $Evidence_t$ — за глосарієм v0.5: reason-with-check, умовна стипуляція.)
...
# 36. Changelog 0.2 — виміри, що змінили модель

Кожна зміна 0.2 зароблена конкретним влучанням аудиту (провенанс: `drafts/EXP-RVB-1-RESULTS.md`); жодна не є смаковою:

| Зміна | Хто заробив |
|---|---|
| §3: типізація $M$ по $(\Delta X \cup \Delta C \cup \Delta I)$ + carrier-відношення для constraints/інваріантів | G1 (релятивізація E1 була холостою без носіїв) → борг D5 |
| §15: $A(x)$ вилучено з диз'юнкції; поділ фізичне/інформаційне визначено відносно $B_{bnd}$ з носіями | V1 і X3 незалежно ($A(x)$ похідне від $x, C$ за §5) |
| §17: representational capacity → Speculative; ослаблена форма заявлена явно | V2 і X4 незалежно; §34 Q4 |
| §20.1: ієрархія інтервенцій $do(\pi)/do(G)/do(H)$ — тест кожного рівня при живих нижчих | G5 (борг D15: $do(\pi)$ вимикає $G$) |
| Шапка: межі Part'ів виправлено (I: §1–14, II: §15–28, III: §29–35) | X4 (застарілий індекс) |
| §3: нотація $B_{bnd}$ проти потрійного перевантаження голого $B$ | V1 (глибина 4), еррата E2 |

Петля "мислення визначає реальність → реальність визначає мислення" замкнена буквально: модель породила протокол вимірювання, виміри переписали модель. Наступній ревізії — той самий шлях: спершу влучання, потім зміна.

## 36.1 Результат аудиту G6 самої ревізії 0.2 (кредитна відомість)

| Ремонт 0.2 | Статус за G6 |
|---|---|
| Межі Part'ів | **зараховано** (не оскаржено) |
| §15: вилучення $A(x)$ | зараховано частково — але поділ, типізований через зовнішнє $M$, **загубив ендогенну зміну** ($T$, $e_t$): камінь §15 випав з обох гілок → **D17** |
| §3: carrier | **НЕ зараховано** → **D16**: ΔX/ΔC/ΔI — сорти без доменів; $carrier$ без кодомена (запис "усередині $S_{in}$" — сортова помилка); §6/§7 не мають carrier-слота |
| §20.1: ієрархія інтервенцій | **НЕ зараховано** → **D18**: критерій не контрастний ($do(G{=}G')$ проти чого?); клас допустимих $G'$ не обмежено (вимірюється свобода моделювальника, не властивість системи); контрприклад: для нерефлексивного player $G = id$, тож нетривіальний $G'$ змінює траєкторію — тест сертифікує рефлексивність, якої нема; $do(G)$ заморожує $H$ — конфаунд D15 поверхом вище; $H$ не має релятума; шість сходинок §18 проти трьох рівнів |
| Процес | **D19**: FLOW казав "закриває D5/D15", поки глосарій із пріоритетом казав "ВІДКРИТО" — зміни FLOW, що торкаються реєстру, віднині йдуть лише разом із версією глосарія |

**Правило 0.2.1, виміряне двома поспіль відхиленими ремонтами (G5, G6):** глибокі концептуальні борги (D5, D15, D16–D18) не закриваються на сесійній швидкості. Вимоги G6 — вхідний контракт наступної ревізії, а не список для сьогоднішнього спринту.

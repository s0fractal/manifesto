...
**Статус:** **ПРИЙНЯТО оператором** (s0fractal, 2026-09-03) — акт класу (c). Прийнято
**зріз, а не текст цілком**.

**Нормативне:** §§1–10 — типи retirement і інваріанти I1–I10; підтриманий
`RetirementRecord v0.1` у рівно двох комбінаціях, які покриті fixture'ами:
`APPLIED + in-repo` і `DRY_RUN + external`. Живий consumer —
`tools/retirement_record_check.py`, у CI.

**Evidence/history, не норма:** Phase 1–2 (`COMPLETE_BY_EXISTING_SPECIMEN`,
загальний consumer) і BOS `DRY_RUN / FORM_LIMIT_FOUND`.

**Явно non-normative і відкрите:** Phase 3–4, кандидатний CLI, зовнішній
`APPLIED`, implementation редакції (`REDACTED`) і §15 відкриті рішення.

Норма не є дозволом на масове прибирання і не є списком файлів-кандидатів.
...
### Phase 2: загальний consumer існує (2026-09-03)

Форма вище перестала бути прозою. `tools/retirement_record_check.py` читає
`RetirementRecord` **як дані**, а не читає прозу про нього; записи лежать у
`drafts/retirement-records/`. Consumer вузький навмисно: два fixture і жодної
спроби покрити retirement взагалі.
...
дірявою в чотирьох місцях; рев'ю власника їх назвало, і вони варті запису, бо всі
чотири — про те, що **green був ширший за предикат**:

1. **argv брався із запису.** `["/usr/bin/true", "<запінений чекер>"]` давало
   `postconditions-replayed=2`, не запустивши чекер жодного разу: entrypoint
   запінений, присутній в argv — і переданий як інертний аргумент до `true`.
   Тепер argv **конструюється консюмером** із закритого `runner` + запіненого
   `entrypoint` + `args`; запис не має слота `argv` узагалі.
2. **Оракул самосертифікувався.** Очікуваний вердикт лежав у самому fixture, тож
   видалення `bos-archive.json` лишало ґейт зеленим. Тепер очікування, множина
   ID і відповідність `filename ↔ id` живуть у **закритому маніфесті всередині
   чекера**, а обидва входи (ґейт і selftest) звіряються з ним.
3. **Malformed-запис падав traceback'ом** (`KeyError: 'subjects'`) замість
   типізованої відмови. Тепер точна перевірка обов'язкових полів **до** будь-якої
   індексації; catch-all лишається останньою мембраною, не схемою.
4. **`APPLIED` не був зв'язаний із переходом загально.** Що суб'єкт зник саме в
   названому apply-коміті, доводив лише специфічний embedded-claims чекер; інший
   запис міг би назвати ранній unrelated child. Тепер consumer сам вимагає: суб'єкт
   існував на before revision **і** відсутній у дереві apply-коміту.

`v0.1` навмисно підтримує лише дві комбінації, які покриті fixture'ами:
`APPLIED + in-repo` і `DRY_RUN + external`. Решта відмовляється, бо невипробуваний
шлях, що повертає `VALID`, вартий менше за той, що відмовляє.

Що в ньому зв'язано: множинні суб'єкти з modes і причинами **на суб'єкт**; digest
...
відмова називає єдину справжню діру, а не ховає купу інших. Зворотний burn: з
видаленою відмовою ґейт дає `MISMATCH bos-archive expected=REFUSED:… actual=VALID`
і виходить із 1.

**Чого consumer навмисно не робить.** Немає глобального retirement-бейджа: вивід
по-записний, а `VECTOR` рахує записи й прямо каже `semantic-credit=none`. `authority`
перевіряється як **адреса акту**, не як доведена правомочність — I6 лишається
governance-питанням, і чекер ніколи не засвідчує, що вилучення було в чиїйсь владі.
`VALID` означає рівно: операнди цього запису зв'язались і його postconditions
відтворились зеленими. Не означає, що вилучати було мудро.

**Відома ціна.** Піновані replacement-операнди червонять запис, коли живі файли
рухаються. Це проявилось у тому ж коміті: правка CI-воркфлоу зламала його ж пін у
записі й вимагала перепіну. Гірший різновид тієї ж пастки: `eb1502c` був зелений
на момент коміту й **червонів одразу після** — zombie-перевірка сканує *tracked*
файли, а перевірки ганялись до `git add`, коли записів ще не було в `git ls-files`.
Записи іменують вилучені суб'єкти за призначенням, тож `drafts/retirement-records/`
тепер виключено зі сканування як tombstone-class, як і сам tombstone. Урок вужчий
за «бути уважним»: **перевірку, що читає `git ls-files`, треба ганяти після
`git add`, бо до нього вона бачить інший світ.** Тертя навмисне — зсунутий replacement має змусити
подивитись, — але якщо воно почне з'їдати більше уваги, ніж ловить, це висновок,
який треба записати, а не привід тихо послабити правило.

**Статус підвищення.** Ґейт, названий власником, зелений на всіх трьох краях.
Підвищення драфту — акт власника, і воно **не взяте**: цей розділ фіксує, що воно
розблоковане, а не що воно відбулось.

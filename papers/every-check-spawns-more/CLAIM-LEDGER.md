...
| C2-MAP | The **eight preserved transcripts selected for the governed 4×2 cohort** (over the four roots used in the crossed comparison) form an **addressable 4×2 observed-model cohort** — the pairs (`root_digest` × `verifier_identity` ∈ {`claude-opus-5`,`claude-sonnet-5`}) are each byte-mapped to the frozen corpus (three evidence-gated spans per transcript). This proves the four ROOT-token × observed-model pairs, **not** that these runs belong to the crossed experiment (run-level 1b/1c provenance is **not** established). It is **address/cohort structure only**: it asserts **nothing** about ô. Deposit strategy `corpus_activation` consumes the live trust root + the independently-verified activation report: `REFUSED: ACTIVATION_NOT_APPLIED` while the trust root is empty; `CHECKED` only after the operator applies the pinned governance diff. Definition: `CORPUS-C2-MAP-CLAIM-0.1.md`. | ADDRESS MAP | CORPUS-C2-MAPPING-0.2.json; CORPUS-C2-MAP-ACTIVATION-0.1.json; activation report | checker (relations) + machine-local (raw spans) | `integrity_break` (a span/id/relation fails to re-derive) OR `bijection_break` (the 4×2 cohort is not exactly covered) | raw-span truth is machine-local until evidence publication; CHECKED attests recomputable relations + a live-activated root, **not** re-execution of the raw bytes in CI |
| C2-MEAS | The **measurement** of C2 — ô (offspring before dedup), the dedup removal decisions, and the per-act ô — is **not replayed** by the address map. offspring/dedup/ô are neither extracted (L2) nor derived (L3) by this path; C2-MAP addressability grants it **nothing**. | MEASUREMENT | EXP-RVB-1-RESULTS.md#crossed | — (permanently refused until measurement is extracted + adjudicated) | `integrity_break` | **permanent** `REFUSED: MEASUREMENT_NOT_REPLAYED`; greening this from C2-MAP would be composition laundering (Codex governance-activation review) |

...
| B7 | One **stored** SKI check re-executes under the released `warrant-verify==0.9.0`: `warrant --store .warrants check 0597575d…` re-runs `(PLUS 74 1) F X → F⁷⁵(X)` to **`pass`, result `e0419cc5…`, `atp_spent=2108`**. **Separately**, the pack **as a whole** is `LEGACY_UNPINNED` (no dependency closure; not strict-replayable). | **UNBOUND OBSERVATION** — stated exactly, **not** checked | `deposit_check.py` B7 = **REFUSED: `WARRANT_ARTIFACT_NOT_BINDABLE`**. Both observations run (CI installs the wheel and prints them), and neither is credit: the engine has no strategy that can bind the artifact producing them. A named interpreter answers its own identity probe; pinning `warrant.py` pins one front module, not the runtime closure that computes the result; and `replay_pack.py` is a verifier inside the tree it verifies. Four reproduced bypasses (deposit_check.py, B7 note) are standing negative controls | external released distribution — **outside** this repository's content binding | observe (by hand or in CI); **no repro path awards this row** | `unbound_credit` (any change that puts B7 into the CHECKED set without an external artifact anchor — an interpreter identity, its runtime closure, and an out-of-tree anchor for the pack verifier) | the claim's **content** is exact and reproducible; its **status** is not earned here. A one-sided check against a constant is not a two-sided equality receipt, adoption, or strict pack replay; neither observation is credit for the other; `warrant verify --settlement` (4 records, 0/0) is not part of this claim |

...
| B7 | One **stored** SKI check re-executes under the released `warrant-verify==0.9.0`: `warrant --store .warrants check 0597575d…` re-runs `(PLUS 74 1) F X → F⁷⁵(X)` to **`pass`, result `e0419cc5…`, `atp_spent=2108`**. **Separately**, the pack **as a whole** is `LEGACY_UNPINNED` (no dependency closure; not strict-replayable). | CONFORMANCE (per-check) + LEGACY-SEALED (pack) | `deposit_check.py` B7 (`warrant_conformance`): the executed artifact is bound to `warrant-verify==0.9.0` (module digest `0e678567…`, owned by that distribution) **before** its output is read; status, result hash and ATP are parsed and compared independently; `replay_pack.py replay → REPLAY: LEGACY_UNPINNED` (exit 1) is required **separately** | permissive/downstream | execute (bound artifact) | `integrity_break` (the stored check fails to re-execute to the stated status/result/ATP — **or** the pack stops answering `LEGACY_UNPINNED`, which refuses B7 just as loudly) | a **one-sided** check against a constant, **not** a two-sided equality receipt, adoption, or strict pack replay; neither observation is credit for the other; `warrant verify --settlement` (4 records, 0/0) is **not** part of this claim and is not checked here |
